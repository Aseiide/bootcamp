...
class NotificationTest < ActiveSupport::TestCase
  ...
  test '.latest_of_each_link returns only one notification of the latest notifications created at the same time in the same link' do
    notification_without_max_id_of_the_latests = Notification.create!(id: 1, user: @receiver, sender: @sender, link: @link, created_at: @now)
    notification_with_max_id_of_the_latests = Notification.create!(id: 2, user: @receiver, sender: @sender, link: @link, created_at: @now)

    latest_notifications_of_each_link = Notification.latest_of_each_link.to_a

    assert_not_includes latest_notifications_of_each_link, notification_without_max_id_of_the_latests
    assert_includes latest_notifications_of_each_link, notification_with_max_id_of_the_latests
  end
end
